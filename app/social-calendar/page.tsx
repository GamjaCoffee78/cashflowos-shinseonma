// 👉 Social Calendar — one month at a time, read-only.
//    Rows are category='social_plan', copied from the SOCIAL CONTENT CALENDAR block of every month
...
const CAPTION = 'What goes out on IG and TikTok, and when — from the Okmaya project sheet. Tags show the channels (IGF feed · IGR reels · IGST stories · REELS TikTok).'

export default async function Page({ searchParams }: { searchParams: Promise<{ m?: string }> }) {
  const today = todayISO()
  const { m } = await searchParams
  const rows = (await getRecords()).filter(r => r.category === 'social_plan' && !!r.due_date)
  if (rows.length === 0) {
    return (
      <>
        <h1 className="ph">{TITLE}</h1>
        <p className="cap">{CAPTION}</p>
        <Empty label="social posts (press Sync now to read the sheet)" />
      </>
    )
  }
  ...
  return (
    <ProductionView
      ...
      title={TITLE} caption={CAPTION} basePath="/social-calendar" editable={false}
    />
  )
}
