...
const HELP_CARD =
  `🤖 <b>${abangName()}</b> — I run your business from your records. Ask me anything:\n\n` +
...
// ============================================================
// answerWithTools — the Abang "brain + hands" loop (§7e).
//
...
async function answerWithTools(chatId: number, text: string, apiKey: string): Promise<string> {
  ...
  const system =
    // 👉 WHO IT WORKS FOR — from abang/config.ts (edit that file, not this one).
    // This is CONTEXT ONLY. The rules below are absolute and come after it on
    // purpose, so nothing in the business profile can widen what Abang may do.
    abangIdentity() +
  ...
  try {
    ...
    console.error('[CFO] Abang tool loop error:', e)
  }
  return answer
}
