...
| **Anthropic Console** | the key for the photo reader + Abang bot | **Must have credit** (~USD $5 / RM23) or every AI surface shows a calm "add your key," and the bot won't answer. |
...
> ### ✅ Day-1 gate (all true): live URL · **locked** phone app (passcode + full-screen install) · **no 7s hang, no horizontal scroll, calm no-key states** · live Abang · the **Expense agent auto-files small / asks big** · the **Vault** agent built + approve/reject/double-tap tested · 1 daily brief. If any regression canary trips, that's a **🔴 finding**.
