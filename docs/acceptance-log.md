...
Code verified: `answerWithTools()` runs an Anthropic `tools` loop (≤4 rounds) over `BOT_TOOLS` = `get_cash_summary(period)` / `list_overdue()` / `search_records(query,category?)` / `escalate`. Tool results wrapped as untrusted `<<<DATA…DATA>>>`. `escalate` (frustrated / out-of-scope / failed-twice) ⇒ hand-off reply + `logRun('abang','escalated',…)`. No full-table dump.
...
- **With-secret digest+brief+sweep ⇒ needs live keys.** Code verified: the brief mirrors the Dashboard's two rows (`getFunnel()` + money row + 🙋 count/list), optional Abang narrative only when a key is set, then the `SCHEDULED` registry sweep creates proposals only.
